...
### 모드 1 — MCP 서버 (API 키 불필요)

Claude Desktop이나 Claude Code에 MCP 서버로 연결하면, Claude 자신이 LLM 역할을 합니다.
`ANTHROPIC_API_KEY` 없이 현재 사용 중인 Claude 구독만으로 동작합니다.
...
# 2. Claude Code에 등록 (한 번만)
claude mcp add pr-review-agent -- node /절대경로/cross-agent/mcp-server.mjs
```

**Claude Desktop에서 쓰는 경우** — `~/.config/claude/claude_desktop_config.json` (macOS: `~/Library/Application Support/Claude/claude_desktop_config.json`):
...
```json
{
  "mcpServers": {
    "pr-review-agent": {
      "command": "node",
      "args": ["/절대경로/cross-agent/mcp-server.mjs"]
    }
  }
}
```

등록 후 Claude에게 자연어로 요청하면 됩니다:

> "main 브랜치 대비 DocCheck 결과 보여줘"
> "HEAD와 main 사이 diff 가져와서 리뷰해줘"

Claude가 MCP 도구(`run_doccheck`, `get_pr_diff`, `get_changed_files`)를 호출해서 결과를 직접 분석합니다.

---

### 모드 2 — Claude Code 슬래시 커맨드 (API 키 불필요)

이 레포를 클론하면 `.claude/commands/review-pr.md`가 포함되어 있습니다.
Claude Code에서 아래처럼 바로 사용할 수 있습니다:

```
/review-pr main
/review-pr HEAD~3
/review-pr origin/develop
```

Claude Code가 `git diff`, `.reviewagent.yml` 읽기, DocCheck 규칙 적용, 코드 분석을 순서대로 수행하고 구조화된 리뷰 결과를 출력합니다.

---

### 모드 3 — GitHub Actions (BYOK, 자동화)

PR이 열릴 때마다 GitHub Actions에서 자동 실행됩니다. `ANTHROPIC_API_KEY`가 필요합니다.

---

## 빠른 시작 (GitHub Actions 모드)
