...
## B. 산출물
- `agent/out/review_result.json` 생성 (필수)
- `agent/out/review_report.md` 생성 (권장)

## C. 품질/테스트 기준
- 최소 1개의 샘플 입력(PR context mock)을 넣었을 때 결과 JSON이 스키마를 만족해야 한다.
- (Actions 운용 시) 워크플로우가 실패 없이 실행되어야 한다.

## D. 게이트(정책)
- `agent/docs/contract.md`가 LOCKED 상태인데 수정되면 PR은 실패 처리되어야 한다.